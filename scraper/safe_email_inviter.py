...
def get_latest_top_jobs(count: int = 3) -> List[Dict[str, str]]:
    """Fetches 3 fresh active jobs to showcase real immediate value to the student."""
    jobs_csv = os.path.join(BASE_DIR, "scraper", "output", "latest_freshersbridge_jobs.csv")
    sample_jobs = [
        {"company": "Accenture", "title": "Associate Software Engineer", "salary": "₹4.5 - ₹6.5 LPA", "url": "https://freshersbridge.in/jobs"},
        {"company": "Google", "title": "Software Engineering Intern", "salary": "₹1.1 Lakh/month", "url": "https://freshersbridge.in/internships"},
        {"company": "TCS", "title": "Ninja & Digital Hiring 2026", "salary": "₹3.6 - ₹7.2 LPA", "url": "https://freshersbridge.in/jobs"}
    ]
        ...
        try:
            ...
            if not df.empty:
                ...
                for _, row in df.head(count).iterrows():
                    real_jobs.append({
                        "company": str(row.get("company", "Top MNC")),
                        "title": str(row.get("title", "Software Engineer")),
                        "salary": str(row.get("salary", "Best in Industry")),
...
def build_email_content(name: str, email: str, jobs: List[Dict[str, str]]) -> Tuple[str, str, str]:
    """
    Generates personal, non-spammy plain-text and HTML email bodies.
    """
    first_name = name.split()[0].title() if name else "there"

    jobs_plain = "\n".join([f"• {j['company']} — {j['title']} ({j['salary']})\n  👉 https://freshersbridge.in/jobs" for j in jobs])
    
    jobs_html = "".join([f"""
    <li style="margin-bottom: 12px;">
        <strong style="color: #0f172a;">{j['company']}</strong> — {j['title']} 
        <span style="background: #e0f2fe; color: #0284c7; padding: 2px 8px; border-radius: 4px; font-size: 13px; font-weight: 600;">{j['salary']}</span><br/>
        <a href="{j['url']}" style="color: #2563eb; text-decoration: none; font-size: 13px;">View drive details & apply &rarr;</a>
    </li>
    """ for j in jobs])
...
"""
    ...
    <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; line-height: 1.6; color: #334155; max-width: 580px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 18px; margin: 24px 0;">
            <p style="margin-top: 0; font-weight: 700; color: #0f172a; font-size: 15px;">🔥 Fresh Verified Drives Active Today:</p>
